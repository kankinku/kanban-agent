...
import { measureElement, useApp, useStdout, useStdin, } from 'ink';
...
function isToolAwaitingConfirmation(pendingHistoryItems) {
    return pendingHistoryItems
        ...
        .some((item) => item.tools.some((tool) => CoreToolCallStatus.AwaitingApproval === tool.status));
}
...
----------------------------------------------------------------
    ...
    const performMemoryRefresh = useCallback(async () => {
        ...
        try {
            ...
            historyManager.addItem({
                type: MessageType.INFO,
                text: `Memory refreshed successfully. ${flattenedMemory.length > 0
    ...
    const { shouldShowWarning: shouldShowRetentionWarning, checkComplete: retentionCheckComplete, sessionsToDeleteCount, } = useSessionRetentionCheck(config, settings.merged, handleAutoEnableRetention);
    ...
    const uiState = useMemo(() => ({
        ...
        historyManager,
        isThemeDialogOpen,
        shouldShowRetentionWarning: shouldShowRetentionWarning && retentionCheckComplete,
    ...
    return (_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(ConfigContext.Provider, { value: config, children: _jsx(AppContext.Provider, { value: {
                    ...
                    }, children: _jsx(ToolActionsProvider, { config: config, toolCalls: allToolCalls, children: _jsx(ShellFocusContext.Provider, { value: isFocused, children: _jsx(App, {}, `app-${forceRerenderKey}`) }) }) }) }) }) }));
};
