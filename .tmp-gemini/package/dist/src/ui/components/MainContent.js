...
export const MainContent = () => {
    ...
    const { pendingHistoryItems, mainAreaWidth, staticAreaMaxItemHeight, availableTerminalHeight, cleanUiDetailsVisible, } = uiState;
    ...
    const pendingItems = useMemo(() => (_jsxs(Box, { flexDirection: "column", children: [pendingHistoryItems.map((item, i) => (_jsx(HistoryItemDisplay, { availableTerminalHeight: (uiState.constrainHeight && !isAlternateBuffer) ||
                    ...
                    : undefined, terminalWidth: mainAreaWidth, item: { ...item, id: 0 }, isPending: true, activeShellPtyId: uiState.activePtyId, embeddedShellFocused: uiState.embeddedShellFocused }, i))), showConfirmationQueue && confirmingTool && (_jsx(ToolConfirmationQueue, { confirmingTool: confirmingTool }))] })), [
        pendingHistoryItems,
    ...
    const renderItem = useCallback(({ item }) => {
        if (item.type === 'header') {
            return (_jsx(MemoizedAppHeader, { version: version, showDetails: showHeaderDetails }, "app-header"));
        }
