...
const renderWithMockedStats = (metrics, width, currentModel = 'gemini-2.5-pro') => {
    ...
    return render(_jsx(ModelStatsDisplay, { currentModel: currentModel }), width);
};
describe('<ModelStatsDisplay />', () => {
    ...
    it('should filter out invalid role names', () => {
        const invalidRoleName = 'this_is_a_very_long_role_name_that_should_be_wrapped';
        const { lastFrame } = renderWithMockedStats({
            models: {
                'gemini-2.5-pro': {
                    api: { totalRequests: 1, totalErrors: 0, totalLatencyMs: 100 },
