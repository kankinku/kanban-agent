...
describe('SessionRetentionWarningDialog', () => {
    ...
    it('renders correctly with warning message and session count', () => {
        const { lastFrame } = renderWithProviders(_jsx(SessionRetentionWarningDialog, { onKeep120Days: vi.fn(), onKeep30Days: vi.fn(), sessionsToDeleteCount: 42 }));
    ...
    it('handles pluralization correctly for 1 session', () => {
        const { lastFrame } = renderWithProviders(_jsx(SessionRetentionWarningDialog, { onKeep120Days: vi.fn(), onKeep30Days: vi.fn(), sessionsToDeleteCount: 1 }));
    ...
    it('defaults to "Keep for 120 days" when there are sessions to delete', async () => {
        ...
        const { stdin } = renderWithProviders(_jsx(SessionRetentionWarningDialog, { onKeep120Days: onKeep120Days, onKeep30Days: onKeep30Days, sessionsToDeleteCount: 10 }));
    ...
    it('should match snapshot', async () => {
        const { lastFrame } = renderWithProviders(_jsx(SessionRetentionWarningDialog, { onKeep120Days: vi.fn(), onKeep30Days: vi.fn(), sessionsToDeleteCount: 123 }));
        // Initial render
