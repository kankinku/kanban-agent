...
import { ApprovalMode, validatePlanContent, processSingleFileContent, } from '@google/gemini-cli-core';
...
- Load testing for session management`;
    ...
    const renderDialog = (options) => renderWithProviders(_jsx(ExitPlanModeDialog, { planPath: mockPlanFullPath, onApprove: onApprove, onFeedback: onFeedback, onCancel: onCancel, width: 80, availableHeight: 24 }), {
        ...options,
        config: {
    ...
    describe.each([{ useAlternateBuffer: true }, { useAlternateBuffer: false }])('useAlternateBuffer: $useAlternateBuffer', ({ useAlternateBuffer }) => {
        ...
        it('bubbles up Ctrl+C when feedback is empty while editing', async () => {
            ...
            const { stdin, lastFrame } = renderWithProviders(_jsx(BubbleListener, { children: _jsx(ExitPlanModeDialog, { planPath: mockPlanFullPath, onApprove: onApprove, onFeedback: onFeedback, onCancel: onCancel, width: 80, availableHeight: 24 }) }), {
                useAlternateBuffer,
                config: {
                    getTargetDir: () => mockTargetDir,
                    getIdeMode: () => false,
...
//# sourceMappingURL=ExitPlanModeDialog.test.js.map