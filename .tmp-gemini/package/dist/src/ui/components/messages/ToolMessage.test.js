...
vi.mock('../GeminiRespondingSpinner.js', () => ({
    GeminiRespondingSpinner: () => _jsx(Text, { children: "MockRespondingSpinner" }),
}));
...
describe('<ToolMessage />', () => {
    ...
    describe('ToolStatusIndicator rendering', () => {
        it('shows ✓ for Success status', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Success }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows o for Pending status', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Scheduled }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows ? for Confirming status', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.AwaitingApproval }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows - for Canceled status', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Cancelled }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows x for Error status', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Error }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows paused spinner for Executing status when streamingState is Idle', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Executing }), StreamingState.Idle);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows paused spinner for Executing status when streamingState is WaitingForConfirmation', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Executing }), StreamingState.WaitingForConfirmation);
            expect(lastFrame()).toMatchSnapshot();
        });
        it('shows MockRespondingSpinner for Executing status when streamingState is Responding', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: CoreToolCallStatus.Executing }), StreamingState.Responding);
            expect(lastFrame()).toMatchSnapshot();
        });
    });
