...
describe('Focus Hint', () => {
    ...
    describe.each(testCases)('$componentName', ({ Component }) => {
        it('shows focus hint after delay even with NO output', async () => {
            const { lastFrame } = renderWithProviders(_jsx(Component, { ...baseProps, resultDisplay: undefined }), { uiState: { streamingState: StreamingState.Idle } });
        ...
        it('shows focus hint after delay with output', async () => {
            const { lastFrame } = renderWithProviders(_jsx(Component, { ...baseProps, resultDisplay: "Some output" }), { uiState: { streamingState: StreamingState.Idle } });
            // Initially, no focus hint
    ...
    it('handles long descriptions by shrinking them to show the focus hint', async () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolMessage, { ...baseProps, description: longDescription, resultDisplay: "output" }), { uiState: { streamingState: StreamingState.Idle } });
        act(() => {
            vi.advanceTimersByTime(SHELL_FOCUS_HINT_DELAY_MS + 100);
        });
