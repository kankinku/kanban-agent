...
import { CoreToolCallStatus, ApprovalMode, AuthType, GeminiEventType as ServerGeminiEventType, ToolErrorType, ToolConfirmationOutcome, MessageBusType, tokenLimit, debugLogger, coreEvents, CoreEvent, MCPDiscoveryState, } from '@google/gemini-cli-core';
...
describe('useGeminiStream', () => {
    ...
    describe('Thought Reset', () => {
        it('should keep full thinking entries in history when mode is full', async () => {
            ...
            const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, fullThinkingSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
            await act(async () => {
                await result.current.submitQuery('Test query');
            });
            expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
