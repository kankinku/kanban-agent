...
describe('useShellCommandProcessor', () => {
    ...
    const renderProcessorHook = () => {
        ...
        function TestComponent({ isWaitingForConfirmation, }) {
            renderCount++;
            hookResult = useShellCommandProcessor(addItemToHistoryMock, setPendingHistoryItemMock, onExecMock, onDebugMessageMock, mockConfig, mockGeminiClient, setShellInputFocusedMock, undefined, undefined, undefined, isWaitingForConfirmation);
            return null;
        }
        ...
        return {
            ...
            rerender: (isWaitingForConfirmation) => rerender(_jsx(TestComponent, { isWaitingForConfirmation: isWaitingForConfirmation })),
        };
    };
    ...
    describe('Background Shell Management', () => {
        it('should register a background shell and update count', async () => {
            ...
            expect(mockShellSubscribe).toHaveBeenCalledWith(1001, expect.any(Function));
        });
        ...
        it('should show info message when toggling background shells if none are active', async () => {
            ...
            expect(addItemToHistoryMock).toHaveBeenCalledWith(expect.objectContaining({
                type: 'info',
        ...
        it('should persist background shell on successful exit and mark as exited', async () => {
            ...
            const exitCallback = mockShellOnExit.mock.calls.find((call) => call[0] === 888)?.[1];
        ...
        it('should persist background shell on failed exit', async () => {
            ...
            const exitCallback = mockShellOnExit.mock.calls.find((call) => call[0] === 999)?.[1];
            expect(exitCallback).toBeDefined();
            if (exitCallback) {
                act(() => {
                    exitCallback(1);
        ...
        it('should NOT trigger re-render on background shell output when visible', async () => {
            ...
            const subscribeCallback = mockShellSubscribe.mock.calls.find((call) => call[0] === 1001)?.[1];
        ...
        it('should hide background shell when waiting for confirmation and restore after delay', async () => {
            ...
            await waitFor(() => expect(result.current.isBackgroundShellVisible).toBe(true));
        });
