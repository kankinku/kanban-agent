...
export function deleteBoard(id) {
  // SQLite foreign_keys=on 일 경우 연관 tasks가 있으면 에러 발생 가능. 
  // 프론트엔드 또는 API 단에서 task가 없는지 확인하거나 캐스케이드(Cascade) 설정 필요.
...
export function getTaskCountInProgress(agentId = null) {
  // WIP는 전체 큐 기준으로 계산 (agentId 필터 제거 → 모든 에이전트의 InProgress/InReview 합산)
...
export function upsertDefaultAgents() {
  // INSERT OR IGNORE: 기존 에이전트가 있어도 누락 없이 모든 기본 에이전트를 확보
  const now = nowIso();
...
export function computeArtifactChecksum(content) {
...
export function addArtifact(input) {
  ...
  const now = nowIso();
  // checksum이 없으면 content → 없으면 summary+location 기반으로 자동 계산한다
...
export function listAllReviewReports(limit = 100) {
